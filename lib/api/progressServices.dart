...
class ProgressServices {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Check if the data contains a 'data' key with a list of progress entries
      if (responseData.containsKey('data')) {
        final List<dynamic> progressData = responseData['data'];
        
        // Validate the structure of progress data
        if (progressData.isNotEmpty && 
            progressData.every((entry) => 
              entry is Map && 
              entry.containsKey('weight') && 
              entry.containsKey('height'))) {
          return progressData;
        }
      }
    ...
    print('Error in getProgressByChildId: $e'); // Log the detailed error
